...
- *redirectUrl* : pour les mises en oeuvre FireTV, ce paramètre doit être nul. Tous les paramètres de cet attribut seront ignorés. 
...
- Le paramètre redirectUrl pour FireTV est défini par le SDK sur l’authentification adobepass://android.app as est géré par l’instance AccessEnabler unique.
...
Si la variable *url* n’est pas utilisé, l’appel réseau obtenu cible l’URL du fournisseur de services par défaut : l’environnement de production de publication d’Adobe.
...
- c. getAuthentication : Le SDK comprend **access_token** dans les paramètres d’appel 

   - mvpd mémorisé : Accédez à setSelectedProvider(mvpd\_id)
   - mvpd non sélectionné : displayProviderDialog
   - mvpd sélectionné : Accédez à setSelectedProvider(mvpd\_id)
...
- e. get/checkAuthorization: Le SDK inclut **access\_token **dans l’en-tête Autorisation : Porteur **access\_token**
...
| access_denied | 401 | access_token n&#39;est pas valide. Le sdk DOIT demander un nouveau access_token. |
