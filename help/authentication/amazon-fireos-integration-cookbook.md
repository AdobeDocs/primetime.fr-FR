...
1. Créez vos fonctions de rappel :
   ...
   - [displayProviderDialog(mvpds)](#$displayProviderDialog)
      ...
      - Déclenché par `getAuthentication()` uniquement si l’utilisateur n’a pas sélectionné de fournisseur (MVPD) et n’est pas encore authentifié. Le `mvpds` est un tableau de fournisseurs mis à la disposition de l’utilisateur.
   ...
   - [navigateToUrl(url)](#$navigateToUrl)
      ...
      - Ignorée dans le SDK AmazonFireOS, la méthode est utilisée sur les plateformes Android où est déclenché par `getAuthentication()` une fois que l’utilisateur a sélectionné un MVPD.  Le `url` fournit l’emplacement de la page de connexion du MVPD.
   - [`sendTrackingData(event, data)`](#$sendTrackingData)

      - Déclenché par `checkAuthentication(), getAuthentication(), checkAuthorization(), getAuthorization(), setSelectedProvider()`.
Le `event` indique quel événement de droit s’est produit ; `data` est une liste de valeurs relatives à l’événement. 
   - [`setToken(jeton, ressource)`](#$setToken)

      - Déclenché par `checkAuthorization()` et `getAuthorization()` après une autorisation d’affichage d’une ressource.
      - Le `token` est le jeton multimédia de courte durée ; le paramètre `resource` est le contenu que l’utilisateur est autorisé à afficher.
   ...
   - [`setMetadataStatus(metadata, key, arguments)`](#$setMetadataStatus)
      ...
      - Le `metadata` fournit les données spécifiques demandées ; la valeur `key` est la clé utilisée dans la variable `getMetadata()` demande; et le `arguments` est le même dictionnaire que celui qui a été transmis à `getMetadata()`.
...
1. Lancement de l’authentification Adobe Primetime
   ...
   2. Appeler` setRequestor()` d&#39;établir l&#39;identification du programmeur; transmettez le `requestorID` et (éventuellement) un tableau de points de terminaison d’authentification Adobe Primetime.
   ...
   Vous disposez de deux options de mise en oeuvre : Une fois que les informations d’identification du demandeur sont envoyées au serveur principal, la couche d’application de l’interface utilisateur peut choisir l’une des deux approches suivantes :</p>
...
1. Appeler [checkAuthentication()](#$checkAuthN) pour rechercher une authentification existante sans initialiser le flux d’authentification complet.  Si cet appel réussit, vous pouvez passer directement au flux d’autorisation.  Dans le cas contraire, passez au flux d’authentification.
...
1. [Facultatif] Appeler [`checkPreauthorizedResources(resources)`](#$checkPreauth) pour vérifier les ressources que l’utilisateur est autorisé à afficher. Le `resources` est un tableau de ressources protégées associées au jeton d’authentification de l’utilisateur.\
...
1. Validez l’authentification et l’autorisation.
   ...
   - If `getAuthorization()` échec : Examinez l’exception générée pour déterminer son type (AuthN, AuthZ ou autre) :
...
1. Appeler [`logout()`](#$logout) pour déconnecter l’utilisateur. \
   ...
   **Remarque :** Le flux de déconnexion diffère du flux d’authentification dans la mesure où l’utilisateur n’est pas tenu d’interagir de quelque manière que ce soit avec WebView. Il est donc possible (et recommandé) de rendre la commande WebView invisible (c’est-à-dire : masqué) pendant le processus de déconnexion.
