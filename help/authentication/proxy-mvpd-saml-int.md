...
Ce document décrit le flux d’authentification SAML pour les intégrations de proxy.  Ces flux dépendent des données de configuration du proxy présentes dans la configuration du serveur d’authentification Adobe Primetime. Le MVPD du proxy envoie ses données de configuration du proxy au serveur d’authentification Adobe Primetime via le service Web proxy d’authentification Adobe Primetime.

## Données de configuration du proxy {#proxy-config-data}

Chaque proxy MVPD fournit des données de configuration de proxy pour leurs MVPD proxy au service Web proxy d’authentification Adobe Primetime.  Les détails de cette opération sont présentés dans la documentation du service Web proxy.   Pour que le flux AuthN SAML fonctionne, les données de configuration du proxy doivent inclure les propriétés suivantes :
...
Pour que l’authentification Adobe Primetime s’intègre à un proxy MVPD, les éléments suivants sont requis :
