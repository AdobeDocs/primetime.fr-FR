...
1. Récupérez les informations d’identification auprès d’Apple.
   ...
   >[!NOTE]
   >
   >Sélectionnez **[!UICONTROL Content Provider]** pour le rôle Principal.
...
1. Générer une demande de signature de certificat.
   ...
   1. Générez votre CSR.
      ...
      >[!NOTE]
      >
      >Les instructions relatives à cette étape se trouvent dans votre *progiciel de déploiement en flux continu FairPlay*, mais sont incluses ici pour votre commodité. Si vous rencontrez des problèmes avec cette partie du processus, consultez les instructions de *FairPlayCertificateCreation.pdf* (dans votre pack de déploiement).
...
1. Téléchargez votre CSR sur le portail des développeurs Apple.
   ...
   1. Cliquez sur **[!UICONTROL Certificates, Identifiers & Profiles]**, puis sélectionnez la liste déroulante **[!UICONTROL iOS, tvOS, watchOS]** en haut à gauche de la page, puis cliquez sur **[!UICONTROL Certificates->Production]** à gauche de la page.
   1. Cliquez sur le bouton **[!UICONTROL +]** en haut à droite de la page pour demander un nouveau certificat. Sélectionnez l&#39;option **[!UICONTROL FairPlay Streaming Certificate]** sous **[!UICONTROL Production]**.

      La boîte de dialogue *Ajouter le certificat iOS* s’ouvre.
   1. Dans *Ajouter le certificat iOS*, téléchargez le fichier CSR que vous avez généré à l’étape 2.b., puis cliquez sur **[!UICONTROL Generate]**.
   ...
   1. Classez dans votre ASK pour terminer la génération de certificats et cliquez sur **[!UICONTROL Continue]**.
   1. Après avoir vérifié que vous avez enregistré votre ASK, cliquez sur **[!UICONTROL Generate]** pour continuer.
...
1. Configurez votre compte ExpressPlay avec vos informations d’identification FairPlay.
   ...
   1. Ouvrez le fichier [!DNL fairplay.cer] à l’aide de l’utilitaire Apple Keychain Access.
   ...
   1. Sélectionnez **[!UICONTROL Export "Your Company Name"]** et enregistrez le fichier [!DNL .p12].
   ...
   1. Cliquez sur **[!UICONTROL DRM SERVICES]** en haut à gauche, puis sélectionnez l&#39;onglet **[!UICONTROL FairPlay]**.
   1. Téléchargez vos informations d’identification FairPlay vers votre compte ExpressPlay.

      1. Créez un fichier texte contenant la valeur de votre ASK (32 caractères doivent être indiqués, par exemple : `1234567890abcdef1234567890abcdef`) et sélectionnez ce fichier pour le téléchargement.
...
Vous pouvez désormais créer des applications iOS ou des pages HTML5 avec la protection de contenu FairPlay et votre certificat [!DNL fairplay.cer] à l’aide du service ExpressPlay pour FairPlay.

<!--<a id="fig_sjr_2pn_sv"></a>-->

![](assets/multi_drm_expressplay_drm_services_web.png)

### Compressez votre contenu pour FairPlay {#package-your-content-for-fairplay}
...
1. Empaquetez votre contenu.
   ...
   Voici un exemple d’assemblage utilisant Adobe Offline Packager. L’outil Packager utilise un fichier de configuration (par exemple, [!DNL fairplay.xml]) qui ressemble à ce qui suit :
   ...
   * `drm_sys` - La solution DRM que vous incluez dans votre pack. Il s&#39;agit de `FAIRPLAY` dans ce cas.
   ...
   * `content_id` - Valeur par défaut.
   ...
   Le fichier M3U8 généré possède un attribut `EXT-X-KEY` qui s’affiche comme suit :
...
Adobe fournit un exemple de serveur de droits ExpressPlay (SEES) qui montre comment exécuter les *droits basés sur le temps* et *de liaison de périphérique*. Cet exemple de serveur de droits est construit sur les services ExpressPlay.
...
Vous trouverez ici les instructions relatives à la mise en oeuvre de la licence et de la lecture d’un client TVSDK iOS : [Activer Apple FairPlay dans les applications TVSDK](../../../programming/tvsdk-3x-ios-prog/ios-3x-drm-content-security/ios-3x-apple-fairplay-tvsdk.md). Vous pouvez également mettre en oeuvre la lecture hors ligne et la rotation des licences pour FairPlay (facultatif).
...
Avant de commencer cette tâche, téléchargez et lisez le document Apple **&quot;Lecture hors ligne avec diffusion en flux continu FairPlay et HTTP Live Streaming&quot;**. Lisez le guide pour savoir comment télécharger des segments de flux de transport (TS) et les enregistrer sur votre ordinateur local.

Mettez en oeuvre la lecture hors ligne pour FairPlay avec ce flux de travail :

1. Téléchargez le segment HLS TS.
1. Demandez la licence de location permanente au serveur FairPlay (voir **&quot;Politique de location permanente FairPlay&quot;**).
...
>
>Voir le document [Utilisation de la diffusion en flux continu HTTP Live](https://developer.apple.com/library/content/documentation/AudioVideo/Conceptual/MediaPlaybackGuide/Contents/Resources/en.lproj/HTTPLiveStreaming/HTTPLiveStreaming.html#//apple_ref/doc/uid/TP40016757-CH11-SW3) pour plus d’informations.
