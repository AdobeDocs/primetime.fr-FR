...
1. Dans votre `Command Line Tools\` dossier local, mettez à jour le `flashaccesstools.properties` fichier pour qu’il fonctionne avec votre serveur.
...
1. Empaquetez votre contenu.
   ...
   Supposons que le fichier vidéo source soit [!DNL sample.mp4]présent. Dans votre `Command Line Tools\` dossier local, exécutez la commande suivante :
