---
description: Cette section décrit la grammaire de l’entrée de configuration, met l’accent sur les options d’entrée valides et non valides et explique comment les champs facultatifs omis sont interprétés.
seo-description: Cette section décrit la grammaire de l’entrée de configuration, met l’accent sur les options d’entrée valides et non valides et explique comment les champs facultatifs omis sont interprétés.
...
Cette section décrit la grammaire de l’entrée de configuration, met l’accent sur les options d’entrée valides et non valides et explique comment les champs facultatifs omis sont interprétés.
...
1. L’ordre des paires définies dans les objets n’est pas fixe ; ainsi, toute permutation des paires est valide.
...
1. Pour chaque paire d’un objet, on suppose qu’il n’existe qu’une seule instance de cette paire dans une instance donnée d’un objet donné.
   ...
   l’instance suivante n’est pas valide, car il existe deux `foo` paires au sein du même objet :
   ...
   De même, vous disposez de deux objets tels que :
...
1. Pour les définitions où une ou plusieurs chaînes peuvent être sélectionnées, traitez les chaînes comme un ensemble dans lequel les entrées de  sont traitées comme une seule entrée. Par exemple, `["foo", "bar", "foo", "baz"]` est équivalent à `["foo", "bar", "baz"]`
...
1. Pour définir des nombres, un espace est utilisé entre les règles (p. ex. `Digit Digits`), mais aucun espace ne doit être utilisé lors de l’application de la règle.
   ...
   Par exemple, si nous exprimons le nombre *cent vingt-trois* par règle NonZeroInteger, il doit être exprimé en `123` lieu et non `1 2 3`, même si la règle contient un espace entre NonZeroDigit et Chiffres.
...
1. Certaines règles autorisent plusieurs formulaires. Dans ce cas, les différents formulaires sont séparés par le `'|'` caractère.
   ...
   signifie qu’une instance de `Foo` peut être remplacée par &quot;A&quot;, &quot;B&quot; ou &quot;C&quot;. Cela ne doit pas être confondu avec un formulaire qui s’étend sur plusieurs lignes ; c&#39; est une fonctionnalité qui rend les formulaires plus longs plus lisibles.
