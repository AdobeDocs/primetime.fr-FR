...
TVSDK 2.7 prend en charge la résolution simultanée de toutes les requêtes d’annonce dans une coupure publicitaire, plutôt que la résolution séquentielle.
...
* Les modifications de la valeur par défaut de WebViewDebbugingWebViewDebbuging sont définies sur False par défaut. Pour l’activer, appelez setWebContentDebuggingEnabled(true) dans l’application.
* Mise à jour de libcurl vers v7.57.0 et OpenSSL vers v1.0.2k.
* Accès au niveau de l&#39;application pour l&#39;objet de réponse VASTIntroduction d&#39;une nouvelle API NetworkAdInfo::getVastXml() qui donne accès à l&#39;objet de réponse VAST à l&#39;application.
...
Remarque : Le basculement ABR, la lecture de l’astuce, l’insertion d’annonces publicitaires, la liaison audio tardive et la sous-segmentation ne sont pas pris en charge pour la lecture au format MP4.
   ...
   * **Lecture plus fluide -** Ces améliorations améliorent l’expérience des utilisateurs :
          ...
          * Sélection adaptative du débit binaire et de la fréquence d’images pendant la lecture de l’astuce, en fonction de la bande passante et de la mémoire tampon
          * Utilisation du flux principal au lieu du flux IDR pour obtenir une lecture rapide jusqu’à 30 ips.
...
Cette section résume le problème résolu dans la publication de TVSDK 2.7.

* ZD#37166 - L’appel de suivi des erreurs est déclenché même lorsque la publicité est lue correctement.
* ZD#37134 - Les identifiants d’annonce erronés sont renvoyés, au cas où l’annonce wrapper(3P) serait présente avec plusieurs publicités dans la réponse VMAP.
...
* ZD #33062 - Le TVSDK se bloque lors de l’occurrence du caractère pipe dans la réponse VAST sous le noeud CDATA
   * L&#39;API setEncodeUrlForTracking de la classe NetworkConfiguration a été supprimée en tant que caractères dangereux dans une URL à coder.
...
* ZD #32256 - Rotation de licence et problème de rotation des clés - Adobe Access.
...
* ZD #33619 - Transfert rapide d’un contenu de liste de lecture en croissance bloqué à l’état de mise en mémoire tampon près du point de production.
   * Traitement de l&#39;affaire lors du passage du point de vie en mode de jeu de tour.
...
* Zendesk#32252 - TVSDK_Android_2.5.2.12 ne semble pas avoir le correctif pour PTPLAY-20269Ce problème a été corrigé et intégré à la branche 2.5.2.
* Zendesk#31806 - Les vignettes Auditude dans PREPARINGPlayer étaient bloquées dans l’état Préparation car le xml de réponse avait une balise vide. Le problème est maintenant résolu.
...
* Flux de résolution Zendesk#32794- 1080P non lu sur Android.
   * Nous avons modifié les méthodes SizeAvailableEvent et getHeight() et getWidth() de SizeAvailableEvent dans la version 2.5 afin de renvoyer la hauteur et la largeur d’image, qui étaient renvoyées par le format multimédia. Il renvoie désormais respectivement la hauteur et la largeur de sortie renvoyées par le décodeur.
...
* Dans le cas d’une réponse VMAP, les appels de publicité dans une seule coupure publicitaire se déroulent simultanément et les coupures publicitaires sont résolues de manière séquentielle.
...
* [Configuration système requise](https://docs.adobe.com/content/help/en/primetime/programming/tvsdk-2-7-for-android/overview/c-psdk-android-2_7-requirements.html)
* [Guide du programmeur Android TVSDK 2.7](https://docs.adobe.com/content/help/en/primetime/programming/tvsdk-2-7-for-android/overview/c-psdk-android-2_7-overview-prod-audience-guide.html)
* [Javadoc Android TVSDK pour référence API](https://help.adobe.com/en_US/primetime/api/psdk/javadoc_2.7/index.html)
* [](https://help.adobe.com/en_US/primetime/api/psdk/cpp/namespaces.html) d’API C++ TVSDK Android C++ - Chaque classe Java possède une classe C++ correspondante et la documentation C++ contient plus de texte explicatif que les Javadocs. Reportez-vous donc à la documentation C++ pour une meilleure compréhension de l’API Java.
