...
Les Notes de mise à jour de TVSDK 1.4 pour iOS décrivent ce qui est nouveau ou modifié, les problèmes résolus et connus et les problèmes de périphérique dans TVSDK iOS 1.4.

## Nouvelles fonctionnalités {#new-features}

**Version 1.4.45**

* Afin de se conformer à Xcode10, TVSDK est passé de &quot;`libstdc++`&quot; à &quot;`libc++`&quot;. Par conséquent, la version minimale prise en charge est iOS 7. Auparavant, il s’agissait d’iOS 6.
...
Aucune nouvelle fonctionnalité n’est ajoutée dans cette version. Pour un  de problèmes résolus, voir Problèmes résolus.
...
* **allowExternalRecording**: Une nouvelle API est introduite pour permettre la mise en miroir des jeux d’air pour un contenu sécurisé. La mise en miroir des jeux d’air est traitée comme un enregistrement. Par conséquent, la valeur allowExternalRecording doit être définie sur &quot;True&quot;, pour autoriser la mise en miroir des jeux d’air ou sur &quot;False&quot; pour arrêter la mise en miroir des jeux d’air pour le contenu sécurisé. Par défaut, la valeur est true.
...
Intégrer et certifier VHL 2.0 dans le SDK iOS TVSDK : Réduisez la barrière dans l’implémentation de VideoHeartbeatsLibrary en réduisant la complexité des API.
...
* **Prise en charge multi-CDN pour CRS** AdsTVSDK prend désormais en charge Multi-CDN pour les annonces CRS. En fournissant des détails FTP pour les annonces CRS, vous pouvez spécifier des emplacements CDN autres que le CDN par défaut détenu par Adobe, tel qu’Akamai.

**Version 1.4.29**

Dans la classe PTSDKConfig, l’API forceHTTPS a été ajoutée.

La classe PTSDKConfig fournit des méthodes pour appliquer SSL sur les requêtes effectuées sur les serveurs Adobe Primetime et de prise de décision, DRM et Video Analytics. Pour plus d’informations, voir les méthodes `forceHTTPS` et `isForcingHTTPS` les méthodes de cette classe. Si un manifeste est chargé via HTTPS, TVSDK conserve l’utilisation du contenu HTTPS et respecte cette utilisation lors du chargement d’URL relatives à partir de ce manifeste.

**Remarque**: Les requêtes envoyées aux domaines tiers, tels que les pixels du suivi des publicités, les URL de contenu et d’annonce et les requêtes similaires, ne sont pas modifiées. Il incombe aux fournisseurs de contenu et aux serveurs d’annonces de fournir les URL prises en charge par le biais du protocole HTTPS.
...
**Remarque**: Le module Nielsen a été supprimé de la version TVSDK, le module TVSDK sera mis à jour prochainement avec un nouveau module d’intégration Nielsen.

* **Abandon de publicité, chaînement de la marguerite dans la logique de sélection de publicité (Zendesk n° 3103)**

Pour les publicités VAST (créatives) avec la règle de secours activée, le kit TVSDK traite une publicité avec un type MIME non valide comme une publicité vide et tente d’utiliser des publicités de secours à sa place. Vous pouvez configurer certains aspects du comportement de secours.

Pour plus d’informations, voir Reprise [publicitaire pour les annonces VAST et VMAP](../programming/tvsdk-1.4-for-ios/ad-insertion/c-psdk-ios-1.4-ad-fallback.md).
...
* **Intégration du SDK Nielsen**
   ...
   * Le SDK TVSDK prend désormais en charge l’envoi de balises mTVR et MDPR ID3 au SDK Nielsen sans intégration personnalisée. Pour commencer, téléchargez le SDK d’application iOS 3.1.2.19 Nielsen.
...
* Billet n° 36294 - Le SDK iOS ne fonctionne pas avec Xcode 10
...
* Billet n° 36321 - Écart observé dans la plage recherchable entre l’instance PTMediaPlayer et AVPlayer à l’état &quot;Lecture&quot;.
* Billet n° 36493 - `libstdc++` Prise en charge sous iOS 12
...
* Billet N° 34683 - Le Temps De Progression De La Lecture De La Publicité Est Négatif
...
* Billet n° 34801 - currentTime et localTime n’étaient pas mis à jour lors de la recherche d’une nouvelle position pendant l’état en pause.
...
* Billet n° 35037 - Piles de lecture avec une URL incorrecte lors du retour d’une insertion de publicité basée sur un signal.
...
* (ZD#32990) - iOS : Contenu lu au lieu de publicités sur certains indices. L’API &#39;selectedMediaOptionInMediaSelectionGroup&#39; qui faisait partie de l’interface AVPlayerItem a maintenant été déplacée sous AVMediaSelection dans iOS 11. Le problème a été résolu à l’aide de cette nouvelle API.
...
* (ZD#34385) - La lecture bloque les URL incorrectes lors du retour d’une insertion d’annonce basée sur un signal.
   ...
   Augmentez le nombre maximal de répétitions pour CustomAVAssetLoaderOperations, de sorte que les lectures du manifeste puissent continuer à s&#39;exécuter.
...
* (ZD#33904) - TVSDK n’est pas enregistré pour les notifications AVFoundation AVAudioSessionMediaServicesWereLostNotification et AVAudioSessionMediaServicesWereResetNotification.
...
* (ZD #32146) - Aucune erreur PTMediaPlayerStatusError n’est reçue pour le contenu HLS Live lors du blocage de la version bêta de développement iOS 11

   Aucune erreur PTMediaPlayerStatusError n&#39;est reçue pour le contenu HLS Live et VOD sur le blocage à l&#39;aide de Charles (Déconnexion et 403)
...
* (ZD#33341) - DRMInterface.h déclenche des avertissements de création dans Xcode 9
   ...
   Correction de deux prototypes de blocs dans DRMInterface.h qui manquaient du mot &#39;void&#39; dans leur de paramètres 
...
* (ZD #32465) - Le lecteur ne peut pas gérer les listes de lecture fusionnées.
   ...
   Appelez la commande endLoadingWithError(avec: Erreur) pour la fondation AV afin d’essayer d’autres flux/déclencher le basculement.
...
* (ZD #33336) - [iOS] TVSDK - Les capsules publicitaires ne sont pas remplies, malgré le nombre suffisant de publicités renvoyées par la roue libre.
...
* (ZD n° 29462) - TremorHub et VOD A&amp;E provoquant un blocage dans les applications iOS
...
Le problème est résolu. Le SDK iOS génère une exception (AUDNetworkAdInfo::initWithAdId) et ne la gère pas. L&#39;exception est due à un identifiant de publicité vide.
...
* (ZD# 28481) - Fermeture de FER en raison de l’ajout d’une clé incorrecte à la fin d’une coupure publicitaire pour ces flux FER
...
* (ZD# 27460) Premier appel publicitaire - POST vers cdn.auditude<span></span>.com renvoyant 403.
...
Ce problème a été résolu en ajoutant l’API assetDuration afin de `PTVideoAnalyticsTrackingMetadata` mettre à jour la durée des ressources pour les flux en direct et de fournir une logique pour la vérification du flux en direct.
...
Ce problème a été résolu en calculant correctement la dérive de la durée sur les listes de lecture .
...
* Zendesk #18518 - Si le nom de la vidéo n’est pas spécifié, le nom du TVSDK est défini par défaut sur Lecteur *PSDK.*
...
* Zendesk #2231 - Erreur renvoyée après la récupération du manifeste non disponible dans MediaPlayerNotification.
...
* Zendesk #18053 - L&#39;application utilisant le SDK se bloque sur Marshmallow

Le SDK TVSDK se bloquait sur le système d’exploitation Android M lorsque la bibliothèque TVSDK utilisait du code néon qui convertissait les couleurs YUV -> RVB. Ce problème a été résolu en mettant à jour les fonctions à l’origine de ce problème à l’aide d’une version non néon du code.
...
* (ZD n° 2751) CSAI et CRS| Améliorer : Gérez les éléments dynamiques dans certaines URL de fichier multimédia.
...
* (ZD n° 2141) Mise en oeuvre d’Analytics pour l’application TreeHouse, ajout de la bibliothèque AdobeAnalyticsPlugin.a pour créer un package.
...
* (ZD #1129) _player.currentItem.audioOptions renvoyant des valeurs vides
...
* La décision d’ignorer se produit sur le  de recherche dans ce cas. Cependant, puisque les durées des segments publicitaires dans le manifeste ne représentent pas exactement la durée réelle de la publicité, la recherche n’est pas précise dans le cadre. Vous voyez donc quelques cadres de la publicité lorsque les stratégies publicitaires sont appliquées.
...
* Le fichier drmNativeInterface.framework ne se lie pas correctement lorsque la  minimale du est définie sur iOS7 (ou version ultérieure).\
   Solution : Spécifiez explicitement le `libstdc++6`.  bibliothèque dylib comme suit : Accédez à ->Build Phases->Link Binary With Libraries et ajoutez `libstdc++.6.dylib`.

* La publicité post-roulée n&#39;est pas insérée pour remplacer l&#39;API.
* La recherche d’une coupure publicitaire (sans en sortir) génère une  et un  une notification de coupure publicitaire
