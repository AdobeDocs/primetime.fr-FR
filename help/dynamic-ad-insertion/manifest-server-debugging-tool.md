...
Lors de l’appel de l’outil de débogage, vous disposez de plusieurs options pour connaître les informations renvoyées par le serveur de manifeste dans les en-têtes HTTP. Les options n’affectent pas les emplacements du serveur de manifeste dans les fichiers journaux.

### Spécification de ptdebug {#specifying-ptdebug}

Lors du lancement de la journalisation du débogage d’une session de serveur de manifeste, vous pouvez ajouter le paramètre ptdebug à l’URL de requête afin de spécifier les options suivantes pour les informations que le serveur de manifeste renvoie dans les en-têtes HTTP :
...
* ptdebug=AdCall uniquement TRACE_AD_*type* (par exemple, TRACE_AD_CALL) enregistre.
...
Voici un exemple de l’en-tête HTTP renvoyé lorsque `ptdebug=Header`. Certaines longues chaînes de chiffres hexadécimaux sont remplacées par `. . .` pour plus de clarté.
...
| record_type | string | Type de  en cours de journalisation |
| autres champs | *** | Dépend du type de  |
...
| delta | integer | Durée (millisecondes) du traitement de la demande |
| module_type | string | Variante, Flux ou VOD |
...
Les enregistrements de ce type consignent les en-têtes HTTP échangés pendant les appels HTTP entre le serveur de manifeste et le client, le serveur d’annonces ou le serveur de contenu. Les champs situés au-delà de TRACE_HTTP_HEADER apparaissent dans l’ordre indiqué dans le tableau, séparés par des onglets.
...
Les enregistrements de ce type consignent les résultats des demandes de publicité du serveur de manifeste. Les champs situés au-delà de TRACE_AD_CALL apparaissent dans l’ordre indiqué dans le tableau, séparés par des onglets.

| Champ | Type | Description |
|--- |--- |--- |
| status | string | Code d’état HTTP renvoyé |
| request_length | integer | Durée (millisecondes) entre la requête et la réponse |
| ad_server__url | string | URL de l’appel publicitaire, y compris les paramètres  du |
...
| avail_length | nombre | Durée (secondes) de la valeur, à partir du repère publicitaire dans le fichier manifeste de contenu (S/O pour VOD) |
| ad_server_response | string | Réponse codée en base 64 à partir du serveur d’annonces |
...
Les enregistrements de ce type consignent les résultats des demandes de publicité indiquées par le type d’enregistrement. Les champs situés au-delà du type d’enregistrement apparaissent dans l’ordre indiqué dans le tableau, séparés par des onglets.
...
| ad_length | integer | Durée (secondes) de la publicité, d’après la réponse du serveur d’annonces |
| ad_content_url | string | URL du fichier manifeste de la publicité, à partir de la réponse du serveur d’annonces |
| ad_content_url_current | string | URL du fichier manifeste de la publicité insérée. Vide pour TRACE_AD_REDIRECT. |
| ad_system_id | string | Système publicitaire, à partir de la réponse du serveur publicitaire (Auditude si non spécifié) |
| ad_id | string | ID de la publicité, à partir de la réponse du serveur d’annonces |
| creative_id | string | ID du créatif, à partir du noeud publicitaire, à partir de la réponse du serveur publicitaire |
| ad_call_id | string | Non utilisé. Réservé pour une utilisation ultérieure. |
| delta | integer | Durée (millisecondes) prise par ce  |
| misc | string | Raison de l&#39;abandon de la publicité |

>[!NOTE]
>
>Les champs ad_content_url_true, ad_call_id et misc sont facultatifs.
...
Les enregistrements de ce type consignent les résultats des demandes de publicité du serveur de manifeste. Les champs situés au-delà de TRACE_TRACKING_URL apparaissent dans l’ordre indiqué dans le tableau, séparés par des onglets.

| Champ | Type | Description |
|--- |--- |--- |
| pts | nombre | Horodatage . Temps passé dans la vidéo pour appeler l’URL. |
| ad_system | string | Système publicitaire (auditude ou roue libre) |
| url | string | URL avec sonnerie |
| status | string | État HTTP renvoyé à partir du ping |
...
Les enregistrements de ce type consignent un élément créatif publicitaire manquant. Le seul champ au-delà de TRACE_TRANSCODING_NO_MEDIA_TO_TRANSCODE apparaît dans le tableau.

| Champ | Type | Description |
|--- |--- |--- |
| ad_id | string | ID de publicité complète `(FQ_AD_ID: Q_AD_ID[;Q_AD_ID[;Q_AD_ID...]`] Q_AD_ID : `PROTOCOL:AD_SYSTEM:AD_ID[:CREATIVE_ID[:MEDIA_ID]`] PROTOCOLE : AUDITUDE,VAST) |

### enregistrements TRACE_TRANSCODING_REQUESTED {#trace-transcoding-requested-records}

Les enregistrements de ce type consignent les résultats des demandes de transcodage que le serveur de manifeste envoie au CRS. Les champs situés au-delà de TRACE_TRANSCODING_REQUESTED apparaissent dans l’ordre indiqué dans le tableau, séparés par des onglets.

| Champ | Type | Description |
|--- |--- |--- |
| ad_id | string | Identifiant publicitaire complet |
...
| _durée | string | Durée de  (secondes) du créatif transcodé |
...
| start | flotter | Temps  du fragment PTS (secondes avec une précision de milliseconde) |
| end | flotter | Temps de fin du fragment PTS (secondes avec une précision de milliseconde) |
...
| Champ | Type | Description |
|--- |--- |--- |
| timestamp | flotter | Durée (en secondes, avec précision 0,001) de la session de lecture pour effectuer un ping sur l’URL de suivi |
| ad_system | string | Système publicitaire (par exemple, auditude) |
| url | string | URL à ping |
...
| start | flotter | Fractionner le temps  du (secondes avec une précision de milliseconde) |
| end | flotter | Heure de fin fractionnée (secondes avec une précision de milliseconde) |

### enregistrements TRACE_WEBVTT_RESPONSE {#trace-webvtt-response-records}

Enregistre ``of ``ce ``type ``journal ``responses ``du ``manifest ``serveur ``sends ``pour ``clients ``l&#39;envoyer `` `answer` ``à  légendes. ``requests ```for```WEBVTT `` Les champs au-delà de TRACE_WEBVTT_RESPONSE &quot;apparaissent dans l&#39;ordre indiqué dans le tableau, `by`onglets séparés.
...
Les enregistrements de ce type permettent au serveur de manifeste de consigner des  et des informations non planifiées lors de l’assimilation de publicités. Le champ situé au-delà de TRACE_MISC se compose d’une chaîne de message. Les messages qui peuvent apparaître sont les suivants :
...
* AdPlacement adManifestURL=*adManifestURL*, lengthSeconds=*secondes*, ignore=*ignore*, redirectAd=*redirectAd, priority=priority***
...
* Ajout d’un cookie pour récupérer le manifeste brut : [cookie]
* Message *d&#39;erreur d&#39;URL* demandé incorrect. (Échec de l&#39;analyse de l&#39;URL de variante)
* URL appelée : URL *renvoyée : code* de réponse. (URL active)
* URL appelée : Code *de retour d’URL : code* de réponse. (URL VOD)
* Conflit détecté lors de la résolution des publicités : soit l&#39;un des  de la gamme intermédiaire, soit l&#39;extrémité de la gamme intermédiaire, se trouve dans la pré-roulage ou pré-roulage contenue dans la gamme intermédiaire (VOD).
* Exception non gérée détectée générée par le gestionnaire pour URI : URL *de* requête.
* Terminé la génération du manifeste de variante. (Variant)
* Terminé la génération du manifeste de variante.
* Exception lors de la gestion de la redirection VAST *URL de redirection *erreur : message *d’erreur*.
* Échec de récupération de la liste de lecture de la publicité pour l’URL *du manifeste de* publicité.
* Échec de la génération du manifeste ciblé. (HLSManifestResolver)
* Échec de l&#39;analyse de la première réponse d&#39;appel de publicité : message *d’erreur*.
* Échec du traitement de *GET|POST *requête de chemin d&#39;accès : URL *de* requête. (VOD/Live)
...
* Trouvé des disponibilités vides. (VOD)
* *nombre *publicités trouvées. (VOD)
* Requête HTTP reçue. (Tout premier message)
* La publicité est ignorée car la différence entre la durée de la réponse de la publicité (*durée de la réponse de la publicité *sec) et la durée de la publicité réelle (*durée réelle *sec) est supérieure à la limite. (HLSManifestResolver)
* La valeur qui n’a fourni aucune valeur d’ID est ignorée. (GroupAdResolver.java)
* La valeur de temps fournie n’est pas valide : *time *pour availId = ID ** de valeur.
* La valeur de durée non valide fournie n’est pas prise en compte : *durée *pour availId = ID ** de valeur.
...
* Requête non valide. La demande de légende doit être effectuée une fois la session établie. (VOD)
* Requête non valide. Une demande de suivi doit être effectuée après l’établissement de la session. (VOD)
...
* Aucune valeur correspondante n&#39;a été trouvée pour l&#39;ID de valeur : ID *de la* valeur. (HLSManifestResolver)
...
* Demande : *URL*.
* Retour d’une réponse d’erreur pour la demande GET, car aucune session de lecture n’a été trouvée. (VOD)
* Renvoi d’une réponse d’erreur pour la demande GET en raison d’une erreur du serveur interne.
* Renvoi d’une réponse d’erreur pour la demande GET spécifiant un fichier non valide : ID *de requête* publicitaire. (VOD)
* Retour d’une réponse d’erreur pour la demande GET spécifiant un ID de groupe non valide ou vide : ID *de groupe*. (VOD)
* Renvoi d’une réponse d’erreur pour la demande GET spécifiant une valeur de position de suivi non valide. (VOD)
* Renvoi d’une réponse d’erreur pour la requête GET avec une syntaxe non valide - URL *de* requête. (VOD/Live)
* Renvoi d’une réponse d’erreur pour une requête avec une méthode HTTP non prise en charge : *GET|POST*. (VOD/Live)
* Renvoi du manifeste à partir du cache. (VOD)
* Le serveur est surchargé. Passez à la demande de raccordement d’annonces publicitaires. (Variant)
* générant le manifeste ciblé. (HLSManifestResolver)
*  génération du manifeste de variante à partir de : URL *du* contenu. (Variant)
* de l’assemblage de publicités dans le manifeste. (VODHLSResolver)
* Tentative d&#39;assemblage de la publicité à *HH:MM:SS*: AdPlacement [URL *du manifeste* publicitaire=URL *du manifeste, duréeSeconds=* secondes *, ignorer=* ignorer *, redirigerAd=rediriger la publicité, priorité=priorité.*** (HLSManifestResolver)
* Impossible d&#39;obtenir des publicités en raison d&#39;un scénario non valide - a renvoyé le contenu sans publicité. (VOD)
* Impossible d&#39;obtenir les publicités : a renvoyé le contenu sans les publicités. (VOD)
* Impossible d&#39;obtenir le de publicité et aucune URL de contenu n&#39;a été fournie. (VOD)
...
| ms_time | integer | Heure à laquelle l&#39;URL de suivi générée par le serveur de manifeste |
