---
description: La structure de l’interface utilisateur est une couche de l’interface utilisateur au-dessus du SDK du navigateur, qui fournit diverses interfaces utilisateur prêtes à l’emploi liées au lecteur vidéo. Vous pouvez créer un lecteur hautement personnalisable en apportant les modifications appropriées à votre environnement.
title: Structure de l’IU
...
# Structure de l’IU {#the-ui-framework}

La structure de l’interface utilisateur est une couche de l’interface utilisateur au-dessus du SDK du navigateur, qui fournit diverses interfaces utilisateur prêtes à l’emploi liées au lecteur vidéo. Vous pouvez créer un lecteur hautement personnalisable en apportant les modifications appropriées à votre environnement.

>[!TIP]
>
>Le comportement visuel (habillage) et de l’interface utilisateur est personnalisable.

Vous pouvez réécrire vos propres comportements ou remplacer la fonctionnalité de certains comportements par défaut. Vous pouvez également réutiliser les comportements fournis avec le SDK en écrivant entièrement les comportements.

## Créer un lecteur de base {#section_30E4812C4DDA4B519C9C837930B6AE45}

`primetimevisualapi.min.js` est la bibliothèque de structure de l’interface utilisateur et toutes ses fonctionnalités sont exposées par le biais du ptp d’objet global. Dans l’exemple suivant, `videoPlayer` crée le lecteur sous-jacent :
...
Pour générer l’objet JSON, le navigateur TVSDK fournit un outil de configuration de l’interface utilisateur. Dans l’outil, vous pouvez sélectionner différents paramètres, puis cliquer sur **[!UICONTROL Test Configuration]** pour vérifier les paramètres, cliquez sur **[!UICONTROL Download Configuration]** pour télécharger les paramètres. Le contenu du fichier téléchargé est utilisé comme objet JSON à transmettre à la variable `ptp.videoPlayer` API.

**Exécution de l’outil Configuration de l’interface utilisateur**:

1. Hébergez les `frameworks` , disponible dans Browser TVSDK, sur un serveur web local.
...
* **Utilisation des API videoBehavior** `ptp.videoPlayer` renvoie la variable `ptp.videoBehavior`, qui vous permet de configurer le lecteur vidéo sous-jacent. Si un paramètre lié à la lecture doit être configuré, vous pouvez utiliser cette option.

  ```js
  player.setAbrControlParameters ({object})
  ```

* **Transmission d’un objet de configuration à la fonction videoPlayer** Lorsque vous utilisez cet objet, le comportement de l’interface utilisateur peut être configuré en plus des paramètres de lecture décrits ci-dessus. L’appelant doit spécifier les paramètres qui doivent être modifiés et le lecteur continuera à utiliser les valeurs par défaut pour les paramètres non spécifiés.
  ...
  Reportez-vous à la section Afficher la structure des objets de configuration ci-dessous pour connaître la structure de l’objet de configuration.

* **Accès à AdobePSDK.MediaPlayer** Vous pouvez utiliser `videoPlayer.getMediaPlayer` dans certains cas d’utilisation avancés où vous devez accéder au lecteur multimédia du navigateur TVSDK.

* **Configuration de l’habillage du lecteur** Pour plus d’informations sur l’habillage du lecteur, voir [Patinage du lecteur](../../browser-tvsdk-2.4/c-psdk-browser-2.4-userinterface/c-psdk-browser-tvsdk-2.4-skin-the-player.md).

## Modifier un comportement par défaut {#section_D5D692638FFF4BEF81F7BE70E438CCE9}
...
>[!NOTE]
>
>Selon la personnalisation souhaitée, vous pouvez remplacer certaines fonctionnalités du comportement ou écrire votre propre comportement. Pour plus d’informations sur les fonctionnalités qui peuvent être remplacées, voir la section [Structure de l’IU](https://help.adobe.com/en_US/primetime/api/psdk/btvsdk-ui-framework/index.html) Documentation de l’API.

## Références {#section_0A76A3F44D8A49B09FE4C83F3FACCB76}

Voici quelques informations de référence supplémentaires :

* **Affichage de la structure des objets de configuration** Il s’agit de la structure d’objet complète qui mentionne tous les comportements par défaut de manière hiérarchique avec les éléments par défaut pour les comportements. Dans l’exemple de configuration, des usines d’IU ont été utilisées pour créer l’élément. Vous pouvez utiliser les mêmes méthodes ou les méthodes que vous préférez pour construire les éléments.

  Vous devez spécifier uniquement les parties que vous souhaitez modifier. Le reste de la fonctionnalité est sélectionné par défaut. Pour commencer, en fonction du cas d’utilisation, vous devez fournir la variable `SingleViewConfigurationObject` ou le `MultiViewConfigurationObject` structure.
...
* **Éléments d’assistance** Ce concept se compose des éléments suivants :

   * **Usine** Pour créer des éléments visuels, vous pouvez utiliser `ptp.factories.simpleButtonFactory`, `ptp.factories.simpleDivFactory`, `ptp.factories.simpleHRFactory`, et `ptp.factories.simpleSliderFactory`. Pour plus d’informations, voir [Interface utilisateur](https://help.adobe.com/en_US/primetime/api/psdk/btvsdk-ui-framework/index.html) Documentation de l’API.

   * **Mixins** Les mixins sont des modules composables qui peuvent être composés dans les comportements pour utiliser des concepts courants. Par exemple, de nombreux composants souhaitent être conscients des modifications qui peuvent avoir un impact sur leur comportement lorsqu’une publicité est en cours de lecture, par exemple. Tous ces éléments ajouteront une `adBreak` classe .
     ...
     Voici comment un comportement peut utiliser ce mixin :
     ...
     Maintenant `customBehavior` peut utiliser toutes les méthodes exposées par `adBreakStyling`, qui dans cet exemple est : `manageAdBreakStyle`. Un autre cas d’utilisation est lorsqu’un mixin peut ajouter des écouteurs d’événement et que dans le gestionnaire, le mixin peut modifier l’élément d’une manière ou d’une autre. Par la suite, les composants qui utilisent ce mixin disposeront automatiquement de cette fonctionnalité.

   * **Utils** Certains utilitaires, tels que `ptp.elementGetter`, qui est utilisé dans la section de configuration et `ptp.deepmerge`, peut vous aider à créer ou à étendre des comportements. Pour plus d’informations, voir [Interface utilisateur](https://help.adobe.com/en_US/primetime/api/psdk/btvsdk-ui-framework/index.html) Documentation de l’API.
