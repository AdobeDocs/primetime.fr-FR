...
Votre application peut déterminer comment gérer cette situation. Par exemple, vous pouvez fermer l’activité du lecteur et diriger l’utilisateur vers l’activité du catalogue. Le événement d’intérêt est le `STATE_CHANGED` événement et le rappel correspondant est la `onStateChanged` méthode. Voici le code qui surveille si le lecteur change son état interne en ERROR :
