---
description: Suspendre et restaurer le lecteur multimédia TVSDK lorsqu’un écran de périphérique est désactivé et activé doit être géré par votre application.
keywords: SurfaceView;Suspend;Restore;BroadcastReceiver
seo-description: Suspendre et restaurer le lecteur multimédia TVSDK lorsqu’un écran de périphérique est désactivé et activé doit être géré par votre application.
...
Suspendre et restaurer le lecteur multimédia TVSDK lorsqu’un écran de périphérique est désactivé et activé doit être géré par votre application.

Vous pouvez gérer les opérations de suspension et de restauration sur `MediaPlayer` le récepteur de diffusion Android pour l’activation/la désactivation de l’écran.

TVSDK ne peut pas déterminer le moment où un fragment (ou  ) se trouve en arrière-plan ou au premier plan. En outre, l’Android `SurfaceView` n’est pas détruit lorsque l’écran de l’appareil est désactivé (mais que le  est en pause). Toutefois, `SurfaceView` cela ** se produit lorsque le périphérique met votre application en arrière-plan. TVSDK ne peut détecter aucune de ces modifications ; elles doivent donc être gérées par votre application.

L’exemple de code suivant explique comment votre application peut gérer la suspension et la restauration de l’écran du périphérique `MediaPlayer` lorsque l’écran du périphérique est activé et désactivé au niveau de l’application :
