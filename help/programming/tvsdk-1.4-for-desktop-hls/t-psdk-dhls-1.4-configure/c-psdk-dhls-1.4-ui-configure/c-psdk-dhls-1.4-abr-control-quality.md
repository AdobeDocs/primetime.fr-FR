...
* Utilisez la classe d&#39;assistance `ABRControlParameterBuilder` pour définir tout sous-ensemble de paramètres (fonctionne sur `ABRControlParameter` en arrière-plan).

* Définissez les paramètres sur la classe `ABRControlParameter`.

## Configuration des débits adaptatifs à l’aide de ABRControlParametersBuilder {#section_3DDE397A7CE445E1832EBAA46CE5C069}

L&#39;utilisation de la classe d&#39;assistance `ABRControlParametersBuilder` est la méthode la plus simple et la plus efficace pour définir des paramètres ABR.

* Le constructeur `ABRControlParametersBuilder` définit tous les paramètres ABR sur les valeurs par défaut de l&#39;objet `ABRControlParameters` sous-jacent.

* Vous pouvez réinitialiser des paramètres ABR individuels au cours de l&#39;exécution, à condition de conserver une référence à la même instance `ABRControlParametersBuilder`.

Cette classe inclut également la méthode d&#39;assistance `toABRControlParameters()`. Utilisez cette méthode pour obtenir une instance de `ABRControlParameters` et la définir sur la propriété `mediaPlayer.ABRControlParameters`. Vos paramètres entrent ainsi en vigueur dans le lecteur.

1. Instanciez la classe d&#39;assistance `ABRControlParametersBuilder` et définissez les paramètres sur Media Player.
...
1. Modifiez les paramètres ABR individuels au moment de l’exécution.
   ...
   Pour conserver vos paramètres précédents, vous devez conserver une référence à la même instance `ABRControlParametersBuilder` que celle créée à l’étape 1.

## Configurez les débits adaptatifs à l&#39;aide de ABRControlParameters {#section_02161FD0A73F40ED9CAE17F9AF850483}
...
Cette capacité de définir des paramètres ABR a été prise en charge avant l&#39;existence de la classe `ABRControlParametersBuilder`, mais cette capacité est toujours efficace pour définir des paramètres ABR au moment de la construction. Cependant, pour modifier des paramètres individuels après la construction, vous devez utiliser la classe `ABRControlParametersBuilder`.
